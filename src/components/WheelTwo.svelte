<script lang="ts">
	...
	let { isSpinning = $bindable(), onResult, sectors }: Props = $props()
	...
	function drawSector(sector: Sector, i: number) {
		...
		ctx.fillText(sector.label.toLocaleUpperCase(), rad - 10, 10)
		ctx.restore()
	}
...
{#snippet scheibe()}
	<div id="wheelOfFortune" class="capitalize">
		<canvas
			bind:this={wheel}
			id="wheel"
			width={dia}
...
<section class="page nwp center pointer-events-none">
	<div class="padded pointer-events-auto rounded-full bg-base-100 shadow">
		{@render scheibe()}
	</div>
</section>
