<script lang="ts">
	let { map, explored, player, exit } = $props()
	let maxW = $state(0)
	let ts = $state(1)
	...
	const draw = async () => {
		...
		for (let y = 0; y < h; y++) {
			for (let x = 0; x < w; x++) {
				...
				if (explored[y][x]) {
					...
					if (map[y][x] == 2) {
						ctx.fillStyle = 'oklch(87% 0.169 91.605)'
					} else if (map[y][x] == 3) {
						ctx.fillStyle = 'oklch(69% 0.17 162.48)'
					}
				}
		...
		ctx.fillStyle = 'oklch(63% 0.237 25.331)'
		ctx.fillRect(exit.x * ts, exit.y * ts, ts, ts)
	}
